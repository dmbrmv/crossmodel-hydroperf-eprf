...
def simulation(data: pd.DataFrame, params: list[float]) -> np.ndarray:
    ...
    for t in range(len(prec)):
        ...
        if prec[t] <= evap[t]:
            ...
            actual_evap = state[0] * (2.0 - store_ratio) * tws / (1.0 + (1.0 - store_ratio) * tws)
            ...
            store_addition = x1 * (1.0 - store_ratio * store_ratio) * tws / (1.0 + store_ratio * tws)
        ...
        percolation = state[0] * (1.0 - 1.0 / np.sqrt(np.sqrt(1.0 + store_ratio / 25.62891)))
        state[0] -= percolation
...
def _route_uh1(state: np.ndarray, ordinates: np.ndarray, input_val: float, x4: float, nh: int) -> None:
...
def _route_uh2(state: np.ndarray, ordinates: np.ndarray, input_val: float, x4: float, nh: int) -> None:
